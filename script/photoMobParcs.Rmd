...
#install_exiftool()
write.csv(photoMeta,"../data/photoMeta.csv", fileEncoding = "UTF-8")
```
...
for (t in wp$time2){
  ...
  print(abs(difftime(photoMetaBoucle$FileModifyDateCET,t)))
  tmp <- which.min(abs(difftime(photoMetaBoucle$FileModifyDateCET,t)))
  #matrice <- rbind(matrice, as.vector(tmp))
  print(tmp)
    # c'est à dire lg photo
  # on récupère l'heure de la photo
  wp$timePhoto [wp$time2 == t] <- photoMetaBoucle$FileModifyDateCET [tmp]
  # pour éviter que la photo soit reprise, on supprime la phot
  print(wp)
  print(photoMetaBoucle [tmp,])
  photoMetaBoucle <- photoMetaBoucle [-tmp,]
  
}
matrice <-  matrice [-1,]
rownames(matrice) <- seq(84,164)
mini <-apply(matrice, 1,min)
mini 
matrice
which(matrice == mini)
limite <- matrice < 60
# c'est pas logique que des photos soient aussi loin et 
wp
photoMeta
```
...
Pb les pts GPS n'ont pas tous une photo
