...
wp <- st_zm(wp,drop = T)
st_write(wp, "../data/mobParc.gpkg", "wp2", delete_layer = T)
```

20 wp pour le 16/02 (retraité en juillet 2025)
-2 pts de départ en mairie
...
6 sorties déc 2023 - 12 déc (24), 8 janvier (7), 13 (1), 16 (18), 19 (1) et 28 (25) février au total

```{r}
photoMeta$jour <- as.POSIXct(photoMeta$FileModifyDate, format="%Y:%m:%d" )
table(photoMeta$jour)
...
### Pot de fleur près de la mairie
...
tournée de verif à la mare à la veuve
...
```{r}
wp <- st_read("../data/mobParc.gpkg", "wp2")
(wpBambou <- wp$time [wp$name == '020'])
```

point photo

```{r}
photoMeta$FileModifyDate [16]
```


```{r}
# 2 minutes de différence
(wpBambou <- wpBambou + 120)
...
```{r}
names(joint)
st_write(joint [, c("time2", "SourceFile", "name", "cmt")],"../data/mobParc.gpkg", "photoFEV2024", delete_layer = T)
...
data <- st_read("../data/mobParc.gpkg", "photoFEV2024")
