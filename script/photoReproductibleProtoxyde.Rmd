---
title: "Projet photo points chaud protoxyde"
author: "B. Maranget"
date: "5/07/2024"
output: 
  html_document: 
...
Reproduction du traitement photoReproductibleVoirie
...
cheminPhoto <- "S:/VOIRIE PHOTO STATIONNEMENT/07_protoxyde/01_fait/"
```
...
```{r}
data <- st_read("../data/protoxyde.gpkg", "protoxyde")
data
```
...
write.csv(meta , "../data/metaProto.csv", fileEncoding = "UTF-8")
```
...
Les photos sont dans ../photoProto et on archive les photos dans traités

```{r}
image_read("../photoProto/MURAD.jpg", density = NULL, depth = NULL, strip = FALSE)
```


```{r}
meta <- read.csv("../data/metaProto.csv", fileEncoding = "UTF-8")
i <- 1
for (i in  c(1:length(meta$FileModifyDate))){
  ...
  image_write(img2, path = paste0("../photoProto/", nom), format = NULL, quality = NULL,
...
meta$FileName
meta$nom <- meta$FileName
write.csv(meta,"../data/metaProto.csv")
...
```{r, eval=F}
# traitement des NA
meta$photoC <- paste0("../photoProto/", meta$FileName)
meta
```
...
m =leaflet() %>%
  ...
  addMarkers(data = data, group= "data", popup = ~adresse, label = data$id) %>%
addPopupImages(image = data$photoC, group = "metasf", height = 300, width = 300) 
```
...
leaflet() %>%
  addProviderTiles("OpenStreetMap.France") %>%
  addMarkers(data = data, group= "data", popup = ~id, label = data$adresse) %>%
addPopupImages(image = data$cheminPhoto, group = "data", height = 300, width = 300) 
```
