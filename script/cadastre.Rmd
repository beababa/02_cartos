...
Version du cadastre de juillet 2022 puis 2023 puis 2025
...
chCadastre <- "03_SOCLE/cadastre2025.sqlite"
```
...
st_write (section, "../data/gros/cadastre.gpkg", "section", delete_layer = T)
...
mf_layout("Sections cadastrales : ordre", "DGST / oct 2025\nDGFiP, 2025")
dev.off()
```
...
```{r}
# attention chemin 03/SOCLE/cadastre2025.sqlite pour travail en mairie
cadastre <- st_read(paste0(chemin,chCadastre), "parcelle_info")
str(cadastre)
```
...
st_write(cadastre, "../data/gros/cadastre.gpkg", "proprio2025", delete_layer = T)
```
...
st_write(agg, "../data/gros/cadastre.gpkg", "agg", delete_layer = T)
```
...
```{r}
cadastre <- st_read("../data/cadastre.gpkg", "proprio2025")
ind <- grep("COMMUNE DE BONDY", cadastre$proprio)
parcellesMairie <- cadastre [ind,]
png("../img/cadastre2025parcellesMairie.png", width = 6000, height = 6000, res = 800)
...
titre <- paste0("Propriétés mairie (",length(parcellesMairie$geo_parcelle) , " parcelles)")
mf_layout(titre, credits = "Cadastre 2025\nMairie / DGST\nSeptembre 2025")
#mf_label(parcellesMairie, "code", overlap = F, cex=0.5)
dev.off()
```

![](../img/cadastre2025parcellesMairie.png)
...
st_write(joint, "../data/gros/cadastre.gpkg", "parcelleMairie", delete_layer = T)
```
...
proprio <- st_read(paste0(chemin,chCadastre), "proprietaire")
str(proprio)
head(proprio)
...
st_write(jointure, "../data/gros/cadastre.gpkg", "pmorale", delete_layer = T)
```

5427 en jointure seulement.
5558
