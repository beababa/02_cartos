...
```{r}
couches <- st_layers("../data/stationnement.gpkg")
couches$name
couches <- st_layers("../data/stationnement1.gpkg")
```
...
for (c in couches){
  tmp <- NULL
  tmp <- st_read("../data/stationnement1.gpkg", nom [i])
  assign(nom [i], tmp)
  i <- i+1
}
```
...
st_write(data_sf, "../data/stationnement.gpkg", "tmp")
```
...
```{r}
st_write(metasf, "../data/stationnement2.gpkg", "metasf")
...
data$jour <- as.Date (data$jour, format = "%Y:%m:%d")
data$jour
l_eclat <- split(data, f=data$jour)
l_eclat [[1]]
eclat <- names(table(data$jour))
```
...
```{r}
metasf$nomN
metasf$nomN2 <- paste0(metasf$nomN,"<b>Date : </b>", metasf$jour, "<br />")  
test$nomN2 <- paste0(test$nomN,"<b>Date : </b>", test$jour, "<br />")
```
...
eclat
pal <- colorFactor(rainbow(7), domain = c(eclat [1], eclat [2], eclat [3], eclat [4], eclat [5], eclat [6], eclat[7]))
pal
rainbow(7)
```
...
```{r}
recherche <- read.csv2("../data/recherche.csv")
recherche <- stationnement[stationnement$nom %in% c(recherche$num),]
