...
leaflet() %>%
  addProviderTiles("OpenStreetMap.France") %>%
  addMarkers(data = metasf, group= "metasf", popup = ~nomN2, label = metasf$nom) %>%
addPopupImages(image = metasf$photoC, group = "metasf", height = 300, width = 300) 
```
...
metasf$jour <- as.Date (metasf$jour, format = "%Y:%m:%d")
metasf$jour
l_eclat <- split(metasf, f=metasf$jour2)
l_eclat [[1]]
...
eclat
pal <- colorFactor(rainbow(5), domain = c(eclat [1], eclat [2], eclat [3], eclat [4], eclat [5]))
pal
```
