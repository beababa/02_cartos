...
Une comparaison avec le tableau des ER amène à de nombreux pb notamment de parcelles manquantes

```{r}
erreur <- read.csv("../data/erreursER.csv", fileEncoding = "UTF-8")
names(erreur)
names(cadastre)
cadastre$code <- gsub(" ", "", cadastre$code)
jointure <- merge(erreur, cadastre, by.y = "code", by.x = "parcelles.manquantes") 
erreur$parcelles.manquantes
jointure
```
...
for (i in 1:nb){
 ...
 mf_label(sel, var = "code", overlap = F)
