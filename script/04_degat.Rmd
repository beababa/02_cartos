...
data <- jointureCommuneS [, c(1, numCol), drop = T]
...
for (i in 1:8){
  ...
  dataCult$num <- dataCult$oui + dataCult$non
  jointure <- merge(communeS$INSEE_COM,dataCult, by = "INSEE_COM")
  aggdata <-  aggregate (dataCult$INSEE_COM, by = list (dataCult$num), sum)
  # pourquoi ça marche plus haut ? à cause de la géométrie
  }
```
