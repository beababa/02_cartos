...
cheminPhoto <- "S:/VOIRIE PLANS//05_SIG/05_PHOTOS/ril/rorcal2026/"
...
```{r}
fic <- list.files(cheminGPX, "24-MAR-26|30-MAR-26")
list.files(cheminGPX)
fic
trace <- NULL
for (f in fic) {
...
st_write(trace,"../data/ril2026.gpkg", "rilMars2026_2", delete_layer=T)
...
trace <- st_read("../data/ril2026.gpkg", "rilMars2026_2")
```
...
```{r}
table(as.Date(trace$time, format="%d" ))
```
...
```{r}
trace <- trace [(as.Date(trace$time, format="%d" )) != "2026-03-10",]
# suppression colonene
trace <- trace [,c(2,5)]
```

repérage campagnes par jour

```{r}
# filtrer le jour
trace$jour <-  as.Date(trace$time, format="%d" )
# rang de l'heure
trace$num <- rank(trace$time)
trace$name [trace$jour == "2026-03-19" & min(trace$num)]
trace$name [trace$jour == "2026-03-23" & min(trace$num)]
```
...
filtre éventuel sur la date

```{r}
photoMetaSel <- photoMeta [substring(photoMeta$FileModifyDate,1,10) == "2026:03:03", ]
...
```{r}
test <- photoMeta$FileModifyDate [35]
photoMetaSel <- photoMeta [photoMeta$FileModifyDate >  test,]
```
...
pt 153 et photo 1301 

On a rajouté un num fonction du rang afin de confirmer.

```{r}
min(trace$name)
max(trace$name)
# Rétablir l'ordre temporel
trace <- trace [order(trace$num),c("name", "time", "num")]
trace <- trace [trace$num!=1,]
pt <- trace [trace$num==1, c("name", "time")]
pt
photo <- photoMeta [photoMeta$nom == "../photo/ril_IMG_1331.JPG",]
photo$heure  - pt$time
```

52 mn diff entre le gps et l'appareil photo. Attention, 42*60 pourles calculs 

```{r}
dif <- photo$heure - pt$time
class(dif)
which(photoMeta$nom == photo$nom)
photoMeta$nom
# verif le résultat doit être nul
photoMeta [7, c("nom", "heure")]
trace$time[1]
photoMeta$heure [7] - trace$time [1] - (dif)
photoMeta$heureAjustee <- photoMeta$heure -( dif )
class(photoMeta$heureAjustee)
photoMeta [7, c("heure", "heureAjustee")]
```
...
On vérifie les pts avec bcp de photo

```{r}
trace [trace$name == '153',]
```
...
fond()
mf_map(joint, add =T)
joint$TERRAIN <- substring(joint$FileName,5,8)
...
```{r}
names(joint)
st_write(joint [, c("name", "nom", "TERRAIN", "FileModifyDate", "heure")],"../data/ril2026.gpkg", "photoMars2", delete_layer = T)
```
...
data <- st_read("../data/ril2026.gpkg", "photoMars2")
...
data <- st_jitter(data)
mf_map(data)
st_write(data,"../data/ril2026.gpkg", "photoMars2", delete_layer = T)
```
