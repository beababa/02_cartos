...
```{r}
zone <- st_read("../data/pluiM3.gpkg", nom2 [7])
summary(zone)
...
```{r}
mf_map(cadastre)
mf_map(type = "typo", zone [zone$code == 'Nc',], var = "LIBELONG", add = T)
mf_map(type = "typo", var="LIBELONG", zone)
zone [zone$geom == "GEOMETRYCOLLECTION",]
mf_map(type = "typo", zone [!is.polygon(zone$geom)],)
library(units)
zone$aire <- drop_units(st_area(zone))
table(zone$aire==0)
zone <- zone [zone$aire!= 0,]
hist(zone$aire)
mf_map(zone [zone$aire < 3,])
zone <- zone [zone$aire > 3,]
```
...
warnings()
mf_map(type = "typo", zone, var = "LIBELONG")
...
agg <- agg [aire > 3,]
mf_map(agg, type ="typo", var="Group.2", border=NA)
mf_label(agg, var = "LIBELLE")
