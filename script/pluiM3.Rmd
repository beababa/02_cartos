...
```{r}
zone <- st_read("../data/pluiM3old.gpkg", "M3_ZONE_URBA")
summary(zone)
...
table(zone$commune) 
mf_map(zone [zone$commune != '930010',])
zone <- zone [zone$commune == '930010',]
...
# recalcul de code
# extraction des 3 premiers caracteres
agg$code <- substring(agg$LIBELLE,1,3)
agg$code <- gsub('[0-9]', '', agg$code)
agg$code <- gsub(" ", "", agg$code)
table(agg$code)
st_write(agg, "../data/pluiM3.gpkg", "zonageAgg", delete_layer = T)
```
