...
https://data.statistiques.developpement-durable.gouv.fr:443/dido/api/v1/datafiles/f3c2f2cb-8fb1-40fd-8733-964247744c9a/spatial/geojson?millesime=2024-01&geoField=DEP

interface permettant le filtre
...
https://www.statistiques.developpement-durable.gouv.fr/catalogue?page=datafile&datafileRid=f3c2f2cb-8fb1-40fd-8733-964247744c9a
...
rep <- Sys.getenv("HOME")
chemin2 <- paste0(rep,"/03_SIG/03_03_Data/10_HABITAT/")
```
...
```{r}
rpls2019 <- read.csv2(paste0(chemin2,"RPLS2019_detail_IDF_DEP_93.csv"), 
                  #fileEncoding = 'UTF-8', 
...
```{r}
rpls2024 <- read.csv2("../data/rpls2024.csv", skip = 1,  dec= ".")
```

9287 en geojson puis en .csv puisque tout est fait
9286 en csv
...
```{r}
rpls2025 <- st_read(paste0(chemin2, "rpls2025.geojson"))
# 9706 logements
# trop long pb géométrie, plus rapide avec le .csv qu'on spatialise
rpls2025 <- read.csv2(paste0(chemin, "rpls2025.csv"), dec = ".", skip = 1)
```
...
names(fic)
head(fic)
# pb pour 2024 sur le nom de voie
table(fic$NOMVOIE)
fic$NOMVOIE <- gsub ("[0-9]", "", fic$NOMVOIE)
# X Y verif également
fic<- fic [!is.na(fic$X),]
# vérifcation pour le nb d'étages et le nb de voies
verif <- function(fic) {
  ind <- grep("NOMVOIE",names(fic))
  # on suppose que le nom voie sera tjrs en 2e position 19e pour 2024 / 18 pour 2025
  #ind <- names(fic)[ind]
...
res <- verif(fic)
# 2 tableaux pour voir le nom des voies et les étages
res
# nb de voie
par(mar = c(5,12,2,2))
...
summary(df)
df <- df [df$Freq > 70,]
barplot(df$Freq, las = 2, horiz = T, names = df$Var1, main = "nb lgts sociaux par rue > 70", cex.names = 0.5)
# agregation par étage
agg <- function(fic){
  ind <- grep("ETAGE", names(fic), ignore.case = T)
...
rpls <- agg(fic)
st_write(rpls, "../data/RPLS.gpkg", "rpls2025", delete_layer = T)
```
...
rpls <- st_read("../data/RPLS.gpkg", "rpls2025")
```
...
cadastre <- st_read("../data/gros/cadastre.gpkg", "proprio2025")
mf_export(rpls, width = 1000, height = 1000, res = 100, filename = "../img/RPLS2025.png", expandBB = c(10,10,30,10))
mf_init(rpls)
mf_theme("candy")
...
#mf_inset_off()
mf_layout(title = "RPLS 2025, nombre d'étages", 
...
```{r}
table(rpls$cle)
rpls <- rpls [!duplicated(st_drop_geometry(rpls)),]
mf_map(rpls)
st_write(rpls, "../data/RPLS.gpkg", "rpls2024", delete_layer = T)
```

Bcp de doublons d'adresse
...
Sur 2025, le CUS ne donne pas le code 11 (PLAI), voir les autres années
