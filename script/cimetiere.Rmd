...
chemin
# verif aucun chemin sur plusieurs section sinon on retrace les chemins... cas sectop, RS
```
...
mf_label(chemin, var ="num", col= "red", halo = T)
```
...
On a besoin d'identifier les allées pour mesurer la distance point origine allée - pt sépulture

2 cas :

- le st_cast sur un chemin allée fixe uniquement le point d'origine

- sur un chemin couvrant plusieurs allées, on voit qu'il faut joindre les pts 2 à 2
...
ptAlleeDep si 2 points pour une allée
...
Même si on a extrait le ptSep de l'allée, nous n'aurons pas l'allée.
...
```{r}
allee <- chemin
allee$num <- as.integer(allee$num)
str(allee)
ptAllee <-  ptChemin
```

A quelle allée appartient le pt sépulture (withdin)
...
# attention cas AE, les allées sont déjà faites
...
# Traitement : mesurer la distance pt sepulture - pt  départ allée
...
### Verif que les points ne sont pas ordonnée
...
mesurer <- function (nb){
  ...
  mf_map(selSep)
  mf_map(ptAllee[ptAllee$allee == nb,], col = "red")
  mf_map(requiem, add = T, col = NA)
  mf_map(selSep, add = T)
  mat <- st_distance(ptAllee [ptAllee$allee == nb,], selSep )
  dist <- as.vector (mat)
  selSep$dist <- dist
  ...
  selSep$num  <- seq
  selSep
  selSep <- selSep[, c( "allee", "num"), drop = F]
  return(selSep)
}
...
i <- 17
for (i in min(allee$num):max(allee$num)){
  print(i)
  tmp <- mesurer (i)
  res <- rbind(tmp, res)
}
