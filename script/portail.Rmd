...
constitution du gpkg que l'on va passer sur le portail.

Exemple de la voirie :

-   arrêté (arrete.gpkg 2026_01)

-   voirie public privé maj (socle 2026 rue)

-   pb voiries sur les 3 rues à partir de raster

-   photos nid de poule Les arrêtés sont traités mois par mois.
    (Nécessité de vérif manuelle)

objectif date / nom / rue dans tableau

reprise du projet fin 2025 = - nom fichier A2024_numArrete - tableau
date / nom / rue - renommer arrêté pas nécessaire
...
31 images pour décembre1 40 decembre2 10 pour le dernier paquet de 2024
...
Attention, la production des pdf a respecté un ordre numérique il est
nécessaire que la lecture des .png respecte ce même ordre sinon coupure
impossible en fin de parcours !

(cela me prend plusieurs jours pour arriver à une solution...)

Solution : on éclate le nom du fichier pour trier sur le nom, puis
l'indice
...
???? il y a des pertes à chaque fois (pas sur decembre 1 et 2 en tous
cas)
...
On regarde si la définition stricte ARRETE ou juste NA permet de repérer
les arrêtes
...
On a donc 11 arrêtés pour dec1 / 13 pour dec2 / 5 pour le paquet / 22
pour dec 2025

Dés cette étape on vérifie et on rectifie en regardant le .pdf

pb titre : manque - le 434 (22e page), - 431 (24e page) - 414 (32e
page) - 412 (37e page)
...
pour dec 2025 (4 fichiers), l'ordre des arrêtés pose pb, correction sur
le nb de car du nom de fichier
...
Vérification : indImage et indOK doivent correspondre / cela permet de
modifier le filtre sur ARTICLE 1
...
En principe, à ce stade, comme il s'agit de la deuxième verif, pas
d'erreur
...
En regardant le pdf on voit page blanche pdf mais pages scanné
...
Pour chq fichier, on a déterminé le nb de pages au moment de la
transformation pdf - png
...
Ainsi pagination contient deb fin nbPages, num arrêté, nom fichier On
peut voir les absences de num d'arrêté
...
L'opération finale consiste à renommer les fichiers . pdf avec le num
d'arrêté et le lieu

Parcours des fichiers pdf pour les éclater et les renommer arrêté par
arrêté.
