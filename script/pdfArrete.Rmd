...
on renomme l'index des fichiers

```{r}
fic <- dir(path = "../data/pdf/", pattern = ".png")
i <- 0
for (f in fic){
  i <- i +1 
  file.rename(paste0("../data/pdf/",f), paste0("../data/pdf/",i, ".png"))
}
```


TODO pourquoi mes arrêtes disparaissent ?


data/pdf est le répertoire temporaire. C'est à partir des images
que l'on travaille.

pour 29 arrêtés, 81 pages arrêtés (mais basse définition)
...
71 pages, mais seules les premières pages nous intéressent.
...
La jointure se fera sur le champs complet (NOM_1_G), on peut modifier le champs (NOM)
...
- parcours de chaque mot ds la liste des lieux
- parcours de la liste des rues pour trouver correspondance avec grep
...
correction sur la liste directement  suite à examen des mots clés (cas autopont)
...
Attention l'indice de lieu ne correspond plus. On fait une jointure par le num d'arrêté pour 
récupérer l'indice du fichier (de l'image.)
...
On identifie uniquement les doublons dans liste_motCle3
Recherche du terme dans le txt d'origine
choix du terme
...
A revoir. qu'arrive t il si on a besoin ailleurs ?
2 tables de ref, une avec les doublons, une sans on aucun doublon
...
On repère la place du mot du jour  et on récupère les 2 mots
d'après
