...
(cela me prend plusieurs jours poru arriver à une solution...)

Solution on éclate le nom du fichier pour trier sur le nom, puis l'indice

```{r}
nbCar <- nchar(ficPNG)
nom <- substring(ficPNG,1,12)
...
images <- map(ficPNG_ordre, magick::image_read)
```
...
ind est l'indice de la lg d'extraction d'arrêté
...
# rajout de 1 pour compter la pg de garde
...
En attendant on rectifie manuellement le fichier

```{r}
paginationFin <- pagination [pagination$num !="",]
paginationFin$fin [13] <- 35
paginationFin$nbPages [13] <- paginationFin$fin [13] - paginationFin$deb [13] 

write.csv(paginationFin ,"../data/pagination.csv", row.names = F)
ficPNG_ordre
```
