...
data <- read.csv2(paste0(chemin,"06_URBA/personne morale opendata/PM_25_NB_930.csv"))
sel <- "BONDY"
...
cadastre <- cadastre [unique(cadastre$code),]
...
head(cadastre [nchar(cadastre$code == 6,), c("code", "adresse")])
...
```{r}
jointure$Dénomination..Propriétaire.s..parcelle.
nom <- "COMMUNE D AVIGNON"  
sel <- jointure [jointure$Dénomination..Propriétaire.s..parcelle. == nom,]
mf_map(sel)
st_write (sel, "../data/avignon.gpkg", "pmorale")
st_write(cadastre, "../data/avignon.gpkg", "cadastre")
png("../img/avignon.png")
mf_map(cadastre)
mf_map(sel, add = T, col = "red")
mf_layout("Avignon, propriété mairie", "data.gouv,2025")
dev.off()
```
