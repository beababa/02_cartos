...
data <- read.csv2("../data/gros/PM_20_NB_840.txt", fileEncoding = "latin3")
sel <- "AVIGNON"
sel <- "BONDY"
table(data$Département..Champ.géographique.)
nom <-  names(table(data$Nom.Commune..Champ.géographique.))
nom [grep("AVI", nom)]
data <- data [ data$Nom.Commune..Champ.géographique.== sel, ]
str(data)
library(sf)
cadastre <- st_read("../data/gros/cadastre-84007-parcelles.json")
...
cadastre$numero
cadastre$section
cadastre$code <- paste0(cadastre$section, cadastre$numero)
data$N..plan..Références.cadastrales.
data$Section..Références.cadastrales.
data$code <- paste0(data$Section..Références.cadastrales.,data$N..plan..Références.cadastrales.)
