...
```{r}
ril <- read.csv("../data/mars2024.csv", header = T, fileEncoding = "UTF-8", na.strings = "")
ril <- ril [,c(1:15)]
...
```{r}
# spatialisation
ril_sf <- st_as_sf(df, coords = c("X", "Y"), crs = 2154 )
mf_map(ril_sf)
# jointure
...
str(ril)
st_write(ril,"../data/ril2024.gpkg", "mars2024b", delete_layer = T)
ril <- st_drop_geometry(ril)
write.csv(ril, "../data/rilMars2024.csv")
```

les zones de sortie

```{r}
zone <- st_read("../data/ril2024.gpkg", "zone")
```
...
st_write(ril, "../data/ril2024.gpkg", "rilMars2024", delete_layer = T)
str(ril)
...
st_write(zone, "../data/ril2024.gpkg", "zonesDeSortie", delete_layer = T)
```
