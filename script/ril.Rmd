...
```{r}
rilCSV <- paste0(chemin, "ril2026_1 .csv")
```
...
```{r}
st_write(ril_sf,"../data/ril2026.gpkg", "mars", delete_layer = T)
ril <- st_drop_geometry(ril)
write.csv(ril, "../data/rilMars2026.csv")
```
...
DBSCAN <- dbscan(rilXY, eps = 500 , MinPts = 11)
...
```{r}
st_write(ril_sf, "../data/ril2026.gpkg", "mars", delete_layer = T)
st_write(env, "../data/ril2026.gpkg", "env_mars", delete_layer = T)
```

```{r}
zone <- st_read("../data/ril2026.gpkg", "env_mars")
inter <- st_intersection(ril_sf, zone [, 1])
inter <- inter [!duplicated(inter$geometry),]
st_write(inter, "../data/ril2026.gpkg", "mars", delete_layer = T)
```
...
```{r}
photo <- st_read("../data/ril.gpkg", "photo")
```
