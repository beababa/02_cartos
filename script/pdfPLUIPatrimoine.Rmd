...
Extraction de toutes les photos des livrets
...
De nombreuses modifications sont apportées (mauvaise qualité du document de départ)
Egalement, un *num* correspondant à l'ordre

```{r}
df <- read.csv("../data/indexPatrimoineCorrige.csv", fileEncoding = "UTF-8")
df$numAd <- substring(df$texte, nchar(df$texte) - 5, nchar(df$texte))
ind <- grep("bis|ter", df$numAd) 

df$numAd
df$numAd2 <- " "
df$numAd2 [ind] <- df$numAd [ind] 
df$numAd2 <- gsub("[^bis|ter]", "", df$numAd2)
table(df$numAd2)
df$numAd <- gsub ("[^0-9]", "", df$numAd)
df$numAd2 <- gsub("ebis|tbis|sbis", "bis", df$numAd2)
df$numAd2 <- gsub("ster", "ter", df$numAd2)
df$rue <- gsub ("[0-9]", "", df$texte)
df$rue
```
...
library(sf)
df$code <- '93010'
df$adresse <- paste0(df$numAd, df$numAd2, " ",df$rue)
adresse
tibble <- geocode_tbl(df,adresse = adresse , code_insee = code)
hist(tibble$result_score)
data <- st_as_sf(tibble [!is.na(tibble$longitude),], coords = c("longitude", "latitude"), crs = 4326)
pb <- tibble [is.na(tibble$longitude),]
pb$texte
st_write(data, "../data/pluiM3.gpkg", "geocodePatrimoineM2", delete_layer = T)
write.csv(pb, "../data/geocodePatrimoineM2pb.csv", fileEncoding = "UTF-8")
```
...
On repart du fic des 3 livrets

```{r}
ficSel <- pdf_subset(paste0(chemin,fic [1]), 10, "../data/fichePatrimoine.pdf")
texte <- pdf_text(ficSel)
texte
```
...
# Annotations préparatoires à la M3

## Extraction adresse et commentaire

```{r}
data <- read.csv2("../data/22072025_CITAVIZ_ANNOTATIONS.csv", fileEncoding = "UTF-8")
data$Observation <- gsub(":","\n", data$Observation)
data$Observation <- gsub("2. ","\n", data$Observation)
eclate <- strsplit(data$Observation, "\n")
data$adresse <- sapply(eclate, "[[",1)
data$adresse
data$detail <- sapply(eclate, "[",2)
data$detail
write.csv(data,"../data/CITAVIZ.csv", fileEncoding = "UTF-8", row.names = F)
```

Encore bcp de modif à la main.


## Géocodage

```{r}
df <- read.csv("../data/CITAVIZmodifie.csv", fileEncoding = "UTF-8")
library(banR)
library(sf)
df$code <- '93010'
tibble <- geocode_tbl(df,adresse = adresse , code_insee = code)
...
pb <- tibble [is.na(tibble$longitude),]
pb$adresse
st_write(data, "../data/pluiM3.gpkg", "geocodeCITAVIZ", delete_layer = T)
write.csv(pb, "../data/geocodeCITAVIZpb.csv", fileEncoding = "UTF-8")
```
