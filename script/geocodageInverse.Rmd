---
title: "Geocodage y compris inversé"
author: "B. Maranget"
date: "18/07/2024"
output: 
  html_document: 
...
cheminPhoto <- ""
cheminPhoto <- paste0(Sys.getenv("HOME"),"/03_SIG/03_03_Data/05_PHOTOS/nids de poule/")
...
```{r}
fic <- list.files(path = cheminPhoto)
rue <- substring(fic,1, nchar(fic)-4)
rue
```
...
```{r}
tab_test <- tibble(x = rue, y='93010', z=rnorm(24))
res <- geocode_tbl(tab_test,adresse=x, code_insee=y)
pb <- res [is.na(res$latitude),]
res <- res [!is.na(res$latitude),]
res <- st_as_sf(res, coords = c("longitude", "latitude"), crs = 4326)
st_write(res,"../data/voirie.gpkg", "nid")
```
...
On place les 3 derniers points à la main.